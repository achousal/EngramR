...
**Triage heuristics for `health-pattern` observations (auto-generated by health gate):**

- Same `failure_pattern` appears in 2+ consecutive reports (2+ observations with same pattern, different dates) → IMPLEMENT (add validation to the source skill)
- High `affected_count` (10+) on first occurrence → IMPLEMENT (systemic issue worth fixing immediately)
- First occurrence with low count (<10) → KEEP PENDING (wait for recurrence signal)
- Health fix skill already resolved all instances (subsequent report shows 0 for this category) → ARCHIVE
- Pattern is in a category with no automated fix (e.g., Three-Space Boundaries) → KEEP PENDING (queue for human review)
...
### Health-Pattern Proposal Template

When a pattern is detected from `health-pattern` observations (grouping rule #7), use this specialized template targeting skill-level preventive fixes:

```
  Proposal [N]: Prevent [failure_pattern] in [source_skill]

  Evidence:
    - [health-obs-1.md] -- [date], [affected_count] instances
    - [health-obs-2.md] -- [date], [affected_count] instances

  Pattern: Recurring health-pattern -- [failure_pattern] detected [N] times
    across [date range]

  Source skill: [skill that creates the failing content, e.g., /reduce, /seed]

  Current behavior:
    [What the source skill currently does that allows this failure
     to occur -- e.g., no title length validation, no slash check]

  Proposed fix:
    [Specific preventive check to add -- e.g., post-write validation
     in validate_write.py, input sanitization in the skill's instructions,
     or a new quality gate in the processing pipeline]

  Scope assessment:
    - Skill to modify: [specific skill file or hook]
    - Check to add: [what the validation does]
    - Expected reduction: [estimated % of this failure pattern prevented]

  What could go wrong:
    [Risk assessment -- false positives, performance impact, edge cases]

  Reversible: yes -- remove the validation check
```
