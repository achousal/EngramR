...
Ask user (3 lines + tip):
```
Which lab (your research group or solo workspace) directory should I scan?
Example: ~/projects/My_Lab/ (single lab) or ~/projects/ (all labs)

Tip: Point to the root and each subdirectory becomes a separate lab.
```
...
```
I see projects directly under ~/projects/ without lab-level grouping:
  ProjectA/, ProjectB/, ProjectC/

Would you like to:
  1. Organize into lab folders first (Recommended)
     I'll walk you through grouping projects by lab, then onboard each lab.
     Creates: ~/projects/Smith_Lab/ProjectA/, ~/projects/Jones_Lab/ProjectB/, etc.
  2. Treat everything as one lab
     All projects register under a single lab entity.
  3. Continue as-is
     I'll ask which lab each project belongs to during onboarding.
```
...
1. Present ALL projects in a single table with a "Lab" column. Ask the user to name their labs and assign each project in one response (one interaction, not per-project):
   ...
   Projects found -- name your labs and assign each project:
     ...
     | ProjectC/    | ________      |
   ```
2. Show the proposed moves and get confirmation:
   ```
   Proposed moves:
     mv ~/projects/ProjectA -> ~/projects/Smith_Lab/ProjectA
     mv ~/projects/ProjectB -> ~/projects/Jones_Lab/ProjectB
   Proceed? (Y/n)
   ```
3. Execute moves with user confirmation. Every move is confirmed.
4. Proceed with standard onboard flow per lab.

**If Option 2:** Prompt for a single lab name, then proceed as standard mode with all projects under that lab.
...
2. Present the list and ask the user to confirm which are labs to onboard.
...
**Statistical convention signals** -- sample up to 5 R/Python analysis files per project (prefer files in `analysis/`, `scripts/`, `src/`):
...
| Containers | presence of `Dockerfile`, `Singularity`, `singularity.def`, `*.sif` |
...
2. If not detected, ask the user:
   ```
   Which institution is this lab affiliated with?
   Example: MIT, Stanford, Max Planck, Johns Hopkins

   (Skip if you'd rather fill in resources manually.)
   ```
...
5. If /learn fails or returns no useful results, fall back to asking the user directly:
   ```
   Could not find faculty profile for {PI Name}. Please provide:
   - Department(s) at {Institution}
   - Center/institute affiliations (optional)
   - External affiliations (optional)
   ```
...
### Step 3a: Institution and Infrastructure

Present ONLY institution and lab-level infrastructure:

```
=== Institution and Infrastructure ===

Lab Profile (auto-detected):
  ...
  External:    {EXTERNAL_AFFILIATIONS or "none"} (from faculty lookup)

Lab Website: {lab_website_url or "not provided"}  (user provided)
...
Lab Infrastructure (auto-detected + institution profile):
  Compute:     {cluster} / {scheduler}       (from job files)
               {additional clusters}          (from web lookup)
  Containers:  Singularity                   (from .sif in project-a)
  Cloud:       --                            (not detected)
  ...
  Resources:   {resources from lab website}   (from lab website)
  Stats:       FDR, alpha=0.05, log2FC, frequentist  (from code)
  Style:       7x5in figures    (from save calls)

  Items marked [dept] are department-relevant. Deselect anything
  your lab doesn't have access to.
```

Use AskUserQuestion: "Does this infrastructure profile look right? Fill in gaps (core facilities, platform access, shared resources, cloud accounts, GPU access, licensed software, collaborations)."

Accept inline corrections. Infrastructure fields flow into the lab entity node (Step 5b2).

### Step 3b: Projects (one lab at a time)
...
### Step 3c: Cross-Lab Connections (multi-lab only)
...
## Step 4: Strategic Questions (optional)

Only for full onboard mode (not `--update`, not single-add). ONE open-ended question:

```
Anything else I should know?
- Research directions not captured by these projects?
- Upcoming datasets, collaborations, or data access applications?
- Cross-project connections worth highlighting?

(Skip if nothing comes to mind -- /reflect will find connections later.)
```

Use AskUserQuestion. The user can type free-form or skip. If skipped, proceed directly to Step 5.
...
Institution Profile (if newly fetched):
  - ops/institutions/{slug}.md

Research Goals (if new directions identified in Step 4):
...
```
Lab conventions stored in projects/{lab_slug}/_index.md. Edit anytime.
Per-project overrides go in individual project notes.
Run /onboard --update to re-scan and refresh detected conventions.
```
