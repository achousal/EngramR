...
2. **`ops/config.yaml`** — processing depth, pipeline chaining, selectivity, extraction categories
   - `processing.depth`: deep | standard | quick
   - `processing.chaining`: manual | suggested | automatic
   - `processing.extraction.selectivity`: strict | moderate | permissive
   - `extraction_categories`: domain-specific extraction table
...
6. **For pipeline execution (--handoff mode):** Proceed directly to file creation. The pipeline needs to proceed without waiting for approval.
   **For interactive execution (no --handoff):** Wait for user approval before creating files.
...
See extraction_categories in ops/config.yaml

### Hypothesis Source Handling

When the source file has `type: hypothesis` in its YAML frontmatter (files in `_research/hypotheses/`), activate hypothesis-specific extraction. Hypotheses contain novel reasoning built ON TOP of literature -- extract the reasoning, not summaries of cited papers.

**Hypothesis extraction categories:**

| Hypothesis Section | Category | Output | What to Extract |
|-------------------|----------|--------|-----------------|
| ## Mechanism | Mechanism claims -- causal assertions about how and why things work | claim | The causal model: what causes what, through which pathway, why |
| ## Literature Grounding | Evidence claims -- what specific studies show | claim (type: evidence) | Only novel synthesis across papers; skip single-paper summaries (those come from reducing the literature note itself) |
| ## Testable Predictions | Prediction claims -- falsifiable statements | claim | Each prediction as a standalone falsifiable proposition |
| ## Assumptions | Assumption claims -- what the hypothesis takes for granted | claim | Hidden assumptions that constrain validity |
| ## Limitations & Risks | Risk/limitation claims -- known failure modes | claim or tension | Failure modes, confounders, boundary conditions |

**Source attribution:** Each extracted claim gets `source: "[[hypothesis-id]]"` (e.g., `source: "[[hyp-20260115-003-mechanism-model-v2]]"`). This creates bidirectional links: hypothesis -> claims via source footer, claims -> hypothesis via wiki-link body text.

**Key constraint:** Do NOT re-extract content that merely summarizes individual cited papers. The literature notes themselves will be reduced separately via the /literature -> /reduce chain. Extract NOVEL reasoning: the mechanism model, the cross-paper synthesis, the predictions, the assumptions. If a sentence could appear verbatim in the cited paper's abstract, skip it.

**Detection:** Check source YAML for `type: hypothesis` OR check if the file path starts with `_research/hypotheses/`.
...
|----------|--------------|-------------|----------------|
| Core domain claims | Direct assertions about the domain | claim | NO |
...
|----------|-----------------|----------|
| Core domain claim | Direct assertion about the domain | -> claim (SKIP selectivity gate) |
...
**For pipeline execution (--handoff mode):** Proceed directly to extraction. The pipeline needs to proceed without waiting for approval.

**For interactive execution (no --handoff):** Wait for user approval before creating files. Never auto-extract.
...
source_class: "[empirical | published | preprint | collaborator | synthesis | hypothesis]"
verified_by: agent
verified_who: null
verified_date: null
created: "YYYY-MM-DD"
...
**Provenance field inference rules:**

- `verified_by`: always `agent` (human verification happens post-extraction)
- `verified_who`: always `null` (set later by human review)
- `verified_date`: always `null` (set later by human review)
- `source_class`: infer from source type:
  - Source is `_research/hypotheses/*` (type: hypothesis) -> `hypothesis`
  - Source is `_research/literature/*` (published papers) -> `published`
  - Source is `_research/experiments/*` -> `empirical`
  - Source is a preprint (bioRxiv, medRxiv, arXiv, SSRN, ChemRxiv, or no DOI) -> `preprint`
  - Source is from conversation, /seed, or agent synthesis -> `synthesis`
  - Source is from a collaborator or inter-lab federation -> `collaborator`
- `confidence`: assess from evidence strength in the source material:
  - `established`: well-replicated, textbook-level, multiple independent confirmations
  - `supported`: multiple lines of evidence, peer-reviewed
  - `preliminary`: single study, limited evidence
  - `speculative`: inference, not directly demonstrated, novel synthesis
...
3. Cross-chunk connections get flagged for /reflect
...
- **NEVER truncate wiki link targets.** Write the full title inside `[[...]]`. Using `[[some claim title...]]` with an ellipsis creates a dangling link. If the title is long, that is fine -- write it in full.
...
**Characters that MUST be replaced with `-` in titles:**

`/ \ : * ? " < > | . + [ ] ( ) { } ^`

**Common patterns requiring replacement:**
...
**YAML quoting:** All frontmatter string values MUST be double-quoted if they contain `: ` (colon-space) or `#`. The hook blocks unquoted values that would silently misparse.

```yaml
# WRONG -- silently misparsed as nested mapping
description: docs: tighten am/pm references

# RIGHT -- value preserved as intended string
description: "docs: tighten am/pm references"
```
...
- The chain: research query -> inbox file -> /reduce -> notes
...
**For interactive execution (no --handoff):** Never auto-extract. Always present findings and wait for user approval.

**For pipeline execution (--handoff mode):** After presenting the extraction report, proceed directly to creating files. Do not pause for approval. The orchestrator is the approval authority.
...
## /reflect
(to be filled by /reflect phase)

## /reweave
(to be filled by /reweave phase)
...
When a source is an empty stub, contains no extractable content, or is entirely off-topic:

1. Do NOT create any claim or enrichment files
2. Mark the extract task as done in `ops/queue/queue.json`
3. Output a handoff block with zero counts:
...
4. Do not stall or loop attempting to find claims. If nothing is extractable after a thorough read, zero is a valid answer.
...
|-----------|---------------|-----|
| New content to process | /reduce | Extraction requires quality gates |
| claim just created | /reflect | New claims need connections |
| After connecting | /reweave | Old claims need updating |
...
- **manual:** Output "Next: /reflect [created notes]" — user decides when to proceed
...
The chaining output uses the hardcoded command names: /reflect, /reweave, /verify.
