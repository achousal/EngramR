...
Draft 2-3 confounders per claim using a canonical lookup table matched to omic layers from `DATA_INVENTORY`:

| Omic Layer | Canonical Technical Confounders |
|---|---|
| Transcriptomics | Library prep batch, RIN, sequencing depth, cell-type composition |
| Proteomics | Ionization efficiency, sample prep batch, MNAR, dynamic range |
| Genomics | Population stratification, coverage, variant calling pipeline |
| Epigenomics | Cell-type composition, bisulfite conversion, batch effects |
| Metabolomics | Ion suppression, instrument drift, metabolite instability |
| Metagenomics | Extraction bias, 16S vs shotgun depth, database completeness |
| Clinical/EHR | Ascertainment bias, coding variability, MNAR, temporal confounders |
| Flow/CyTOF | Instrument drift, antibody lot variation, gating subjectivity |

Match omic layer from `DATA_INVENTORY` rows to each orientation claim via its linked goal or domain keywords. Refine confounder selection using these additional signals:
...
- **Species mix:** mouse-only datasets -> translational gap claim. Mixed human/mouse -> cross-species challenge claim.
