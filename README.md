...
bridged -- Lab A's methodological observation connects to Lab B's independent
finding from a different dataset. Neither lab would have made that connection alone.
...
6. Start Claude Code in the vault directory. Run `/onboard` to set up your
   lab (or solo workspace), then `/init` to seed your knowledge graph with
   orientation claims, confounders, and assumption inversions.
...
2. **Seed the knowledge graph.** Run `/init` to walk through guided seeding
   for each research goal -- orientation claims, analytical methods,
   confounders, and assumption inversions. This builds the structural
   foundation that later claims and hypotheses connect to. Use
   `/init --cycle` at cycle transitions to refresh inversions and reconcile
   priorities.

3. **Feed it literature.** Drop sources into `inbox/` and `/reduce`, or use
   `/literature` to search PubMed/arXiv directly.

4. **Let the loop run.** `/research` orchestrates hypothesis generation,
   debate, and ranking based on your registered goals and accumulated
   evidence.

5. **Configure the daemon** (optional). Edit `ops/daemon-config.yaml` and
   run in tmux:
   ```bash
...
- **Add a CLAUDE.md** to each project. `/onboard` reads it to auto-populate
  ...
  Statistical modeling of treatment response predictors across
  three intervention protocols.
  ...
  ## Data
  - Measurements: data/observations.csv (12,000 records x 45 variables)
  - Participant metadata: data/participants.csv (n=120)
  ```
...
| `/init`       | Guided knowledge seeding for new vaults and cycle transitions         |
