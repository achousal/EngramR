...
|----------|----------|-----------------|
| `OBSIDIAN_API_KEY` | Yes | Obsidian > Settings > Local REST API > API Key |
| `OBSIDIAN_API_URL` | No | Default: `https://127.0.0.1:27124` |
| `NCBI_API_KEY` | For /literature | [NCBI Account Settings](https://www.ncbi.nlm.nih.gov/account/settings/) |
| `NCBI_EMAIL` | For /literature | Required by NCBI API policy |
...
4. Enable it and note the API key (copy to `.env`)
5. (Recommended) Install **Dataview** for dynamic queries
...
On first session, run:

```
/onboard
```

This bootstraps your lab integration: scans for projects, creates data inventory, links hypotheses to goals.
...
| `/seed` | Create a single claim from conversation |
...
**"OBSIDIAN_API_KEY not set"** -- Copy .env.example to .env and fill in the API key from Obsidian's Local REST API settings.
