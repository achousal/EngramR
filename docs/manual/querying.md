...
rg '^description:.*inflammation' notes/
...
rg -l '^goal:.*ad-biomarkers' _research/hypotheses/
...
1. **Notice the need** -- you keep wanting to filter by tissue type, cohort, or omic layer.
...
4. **Query immediately** -- `rg '^tissue:.*hippocampus' notes/` just works.
