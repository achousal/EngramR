...
#### /seed
Create a new claim from conversation or thinking.

| Property | Value |
|----------|-------|
| Input | Claim title as argument |
| Output | Single claim note in notes/ |

The one exception to pipeline routing. Creates a claim directly in notes/ because the source is conversation, not a document. Still applies quality standards: prose-as-title, description quality, schema compliance.
...
#### /onboard
Guided vault setup and orientation for new users.

| Property | Value |
|----------|-------|
| Input | None (interactive) |
| Output | Initialized vault with guided walkthrough of key concepts and workflows |

Walks a new user through vault structure, session rhythm, claim creation, and the processing pipeline. Checks prerequisites and provides step-by-step orientation.
