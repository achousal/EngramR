...
**4 generation modes:**
1. **Literature synthesis** -- search PubMed/arXiv, identify gaps, propose hypotheses with citations.
...
2. **Literature review** -- search for the claim in PubMed/arXiv, flag if already published or contradicted.
...
#### /literature
Search PubMed and arXiv, create structured literature notes.
...
Searches NCBI EUTILS and arXiv Atom API. Presents results as a table. Saves selected papers as structured notes with frontmatter (type, title, doi, authors, year, journal, status) and sections (Abstract, Key Points, Methods Notes, Relevance, Citations).
...
Auto-redacts PII columns (SubjectID, PatientName, MRN, SSN, Email, etc.) before analysis. Computes summary statistics, correlations, distribution properties. Generates themed plots (histograms, correlation heatmap, missing data chart). Saves report to vault.
...
|----------|-------|
| Invoked by | User (standalone) |
| Reads | User-provided data, _code/styles/PLOT_DESIGN.md, _code/styles/STYLE_GUIDE.md |
...
Reads source material through the bioinformatics lens. Extracts claims across 6 categories: claims (testable assertions), evidence (data points), methodology comparisons, contradictions, open questions, design patterns. Each extracted claim must pass the quality bar: title works as prose, description adds beyond title, specific enough to disagree with.
...
Hybrid scan + interview that registers a research lab (or solo workspace). Scans the lab directory for projects, detects infrastructure (CLAUDE.md, git, tests), builds a data inventory, and creates research goals. Runs once per lab; use `--update` for incremental re-scans. Natural next step after /onboard is /init.
...
**Seed mode** (default): guided knowledge seeding through four phases -- goal selection, domain orientation (core questions as testable claims), methodological seeding (confounders, analytical methods, data realities), and assumption inversions (falsification conditions). Uses /onboard artifacts (data inventory, project metadata, lab conventions) to pre-populate interviews.
