...
1. **`/onboard`** -- Scans your lab (or solo workspace) directory, registers projects, builds a data inventory, and creates research goals. This builds the project infrastructure that everything else operates on.

2. **`/init`** -- Seeds your knowledge graph with foundational claims: orientation questions, methodological knowledge, confounders, and assumption inversions. Uses the artifacts created by /onboard to pre-populate the guided interview.

After both complete, you have a structured foundation. The rest of this page covers your first working session.
