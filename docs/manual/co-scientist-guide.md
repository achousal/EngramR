---
description: "Co-scientist hypothesis pipeline -- generation modes, tournament ranking, evolution, and the self-improving loop"
type: manual
created: 2026-02-21
---
...
cd _code
cp .env.example .env
# Fill in API keys for your domain (all optional -- see _code/README.md)
```

Obsidian is optional for browsing the vault graph but not required for operation.
...
- `/literature` -- Search configured backends (PubMed, arXiv, S2, OpenAlex), optionally enrich results with CrossRef/Unpaywall metadata, save structured notes to `_research/literature/`
- `/eda` -- Run EDA on a dataset (auto-redacts PII columns), saves report to `eda-reports/`
- `/plot` -- Generate figures using the canonical theme from [STYLE_GUIDE](_code/styles/STYLE_GUIDE.md) and [PLOT_DESIGN](_code/styles/PLOT_DESIGN.md)
...
| `_research/literature/` | Structured literature notes from configured search backends |
...
2. **Literature-seeded** -- Search configured search backends, identify gaps, propose hypotheses addressing them
...
1. **Plausibility** -- Assess mechanistic coherence and domain plausibility
2. **Novelty** -- Search configured backends to verify novelty claims against existing literature
