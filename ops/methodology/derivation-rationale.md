...
The system was originally derived for bioinformatics. It is now domain-agnostic via profiles (`_code/profiles/`). The bioinformatics profile remains the reference implementation and default. Search backends, PII patterns, confounders, and data layer heuristics are all profile-configurable. This addendum records the transition; the derivation history above is preserved unchanged.
