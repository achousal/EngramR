# plot_helpers.R -- Color palettes, axis helpers, and save utilities
#
# Re-exports SEX_COLORS, DX_COLORS from palettes.R for backward compatibility.
# Provides y-axis scale helpers and an extended save_plot().
#
# Depends on: ggplot2, palettes.R (for canonical color definitions)

# -- Backward-compatible color re-exports --------------------------------------
# These are re-exported from palettes.R. If palettes.R is not sourced first,
# fall back to inline definitions so existing scripts don't break.

if (!exists("SEX_COLORS")) {
  SEX_COLORS <- c("Male" = "#377EB8", "Female" = "#E41A1C")
}
if (!exists("DX_COLORS")) {
  DX_COLORS <- c("P-" = "#4DAF4A", "P+" = "#E41A1C")
}
