...
def export_claim(
    ...
    *,
    ...
    return ExportedClaim(
        ...
        verified_date=(
            str(fm.get("verified_date", "")) if fm.get("verified_date") else ""
        ),
        tags=[t for t in fm.get("tags", []) if isinstance(t, str)],
        body=_strip_wiki_links(body).strip(),
...
def import_claims(
    ...
    *,
    ...
    for claim in claims:
        filename = _safe_filename(claim.title) + ".md"
        ...
        fm: dict[str, Any] = {
            ...
            "source_vault": claim.source_vault,
            "imported": claim.exported,
        }
        if claim.source:
            fm["source"] = claim.source
        if claim.source_class:
            fm["source_class"] = claim.source_class
        if claim.verified_by:
            fm["verified_by"] = claim.verified_by
        if claim.verified_who:
            fm["verified_who"] = claim.verified_who
        if claim.verified_date:
            fm["verified_date"] = claim.verified_date
        if claim.tags:
            fm["tags"] = claim.tags
        if quarantine:
            fm["quarantine"] = True
        ...
        body = claim.body if claim.body else ""
        content = f"---\n{fm_str}\n---\n\n{body}\n"
...
def _safe_filename(title: str) -> str:
    """Convert a claim title to a safe filename (no path-breaking chars)."""
    # Replace filesystem-unsafe characters with hyphens
    unsafe = r'[/\\:*?"<>|.+\[\](){}^]'
    return re.sub(unsafe, "-", title).strip("-")
