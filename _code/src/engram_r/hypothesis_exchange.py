...
from engram_r.schema_validator import normalize_text, sanitize_title, validate_note
...
def export_hypotheses(
    ...
    *,
    ...
    for note_file in sorted(hyp_path.glob("*.md")):
        ...
        hyp = export_hypothesis(content, source_vault=source_vault, now=ts)
...
def import_hypotheses(
    ...
    *,
    ...
    for hyp in hypotheses:
        ...
        fm: dict[str, Any] = {
            "type": "foreign-hypothesis",
            "title": normalize_text(hyp.title),
            "id": hyp.id,
            "status": hyp.status,
            "elo_federated": 1200,
            "elo_source": hyp.elo,
            "matches_federated": 0,
            "matches_source": hyp.matches,
            "generation": hyp.generation,
            "research_goal": hyp.research_goal,
            "source_vault": hyp.source_vault,
            "imported": hyp.exported,
            "tags": ["foreign-hypothesis"] + hyp.tags,
        }
