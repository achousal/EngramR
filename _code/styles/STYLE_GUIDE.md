...
Every analysis script produces a companion plain-text stats report. This is the canonical pattern (established in AD_Chloe).
...
DAC_CRF's `make_table1(df, output_path)` is the reference implementation (Python, `analysis/table1.py`). For R projects, use `gtsummary::tbl_summary()` or `tableone::CreateTableOne()` to produce equivalent output.
...
DAC_CRF's `na_summary()` in `analysis/load_clean.py` is the reference implementation. For R projects:
