...
> **Scope**: This document is an AD-pipeline-specific supplement. The canonical high-level style guide lives in [`STYLE_GUIDE.md`](./STYLE_GUIDE.md).

## Global Theme (`theme_ad()`)

| Detail | Value | Rationale |
|---|---|---|
| Base theme | `theme_classic` | Clean, no gridlines -- publication standard |
| Base font size | 14pt | Legible at PDF export sizes |
| Title | **bold** | Visual hierarchy |
| Strip background | grey90, no border | Subtle facet labels without heavy frames |
| Strip text | **bold** | Readable facet labels |
| Legend position | bottom | Keeps plot area maximized |
...
**Rationale:** Scatter plots show associations -- forcing y=0 wastes space when biomarker
values live far from zero. Box/violin plots show distributions where the zero baseline
is meaningful. Per best practice: prioritize showing the data's range in scatters, keep
zero origin for distribution plots.
...
## Box/Violin Plots (via `build_*` helpers)
...
| Violin alpha | 0.35, untrimmed | Shows full distribution shape |
| Violin linewidth | 0.4 | Subtle outline |
| Jitter points | size 1.5, alpha 0.6, jitter.width 0.1 | Individual data visible without obscuring violin |
| Mean marker | shape 95 (dash), size 6, black | Mean line across violin |
| Dodge width | 0.7 | Male/Female side by side within each x group |
...
| Box width | 0.35 | Narrow boxes for clean dodged layout |
| Outlier shape | NA (hidden) | Jitter points replace outlier markers |
| Box alpha | 0.35 | Semi-transparent to show jitter underneath |
| Box linewidth | 0.6 | Slightly heavier than violin outline |

### Shared across box/violin
