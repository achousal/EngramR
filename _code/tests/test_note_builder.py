...
class TestBuildLabNote:
    def test_basic_structure(self):
        note = build_lab_note(
            ...
            departments=["Neurology", "Neuroscience"],
            center_affiliations=["Alzheimer's Disease Research Center"],
            external_affiliations=["James J. Peters VA Medical Center"],
        ...
        assert fm["departments"] == ["Neurology", "Neuroscience"]
        assert fm["center_affiliations"] == ["Alzheimer's Disease Research Center"]
        assert fm["external_affiliations"] == ["James J. Peters VA Medical Center"]
...
class TestBuildClaimNote:
    ...
    def test_basic_structure(self):
        stem, content = build_claim_note(
            ...
            description="Ceramide accumulation in temporal cortex correlates with amyloid burden",
        ...
        assert "smith-2024-ceramides" in fm["source"]
        assert "The evidence shows progressive accumulation." in content
        assert stem == "ceramide levels increase in AD brains"

    def test_title_sanitization(self):
        stem, content = build_claim_note(
            title="APP/PS1 mice show amyloid (beta)",
        ...
        assert "(" not in stem
        assert ")" not in stem
        assert stem == "APP-PS1 mice show amyloid -beta"
    ...
    def test_footer_with_relevant_notes(self):
        _, content = build_claim_note(
            ...
            description="desc",
            relevant_notes=[
                ("amyloid drives ceramide", "mechanistic basis"),
                ("ceramide activates PP2A", "downstream effect"),
            ],
            today=date(2026, 2, 26),
        )
        assert "[[amyloid drives ceramide]] -- mechanistic basis" in content
        assert "[[ceramide activates PP2A]] -- downstream effect" in content

    def test_footer_with_topics(self):
        _, content = build_claim_note(
            title="test",
            description="desc",
            topics=["lipid-biomarkers-ad"],
            today=date(2026, 2, 26),
        )
        assert "[[lipid-biomarkers-ad]]" in content
